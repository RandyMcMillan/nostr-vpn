...
struct UserspaceWireGuardPeerRuntime {
...
pub(crate) struct UserspaceWireGuardPeerConfig {
...
pub(crate) struct UserspaceWireGuardRuntime {
    peers: Vec<UserspaceWireGuardPeerRuntime>,
}

impl UserspaceWireGuardRuntime {
    pub(crate) fn new(
        private_key_base64: &str,
        peers: Vec<UserspaceWireGuardPeerConfig>,
    ) -> Result<Self> {
        ...
        for (index, peer) in peers.into_iter().enumerate() {
            let peer_public_key = decode_public_key(&peer.public_key_base64)?;
            let routes = peer
                .allowed_ips
                .iter()
            ...
            runtimes.push(UserspaceWireGuardPeerRuntime {
...
#[cfg(test)]
mod tests {
    use super::{UserspaceWireGuardPeerConfig, UserspaceWireGuardRuntime};
    ...
    fn longest_prefix_route_wins() {
        ...
        let mut runtime = UserspaceWireGuardRuntime::new(
            &local_keys.private_key,
            vec![
                UserspaceWireGuardPeerConfig {
                    participant_pubkey: "general".to_string(),
                    public_key_base64: general_peer.public_key,
                ...
                UserspaceWireGuardPeerConfig {
                    ...
                    public_key_base64: specific_peer.public_key,
    ...
    #[test]
    fn peer_status_reports_completed_handshake() {
        let alice = generate_keypair();
        let bob = generate_keypair();
        let alice_endpoint = SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::LOCALHOST, 42101));
        let bob_endpoint = SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::LOCALHOST, 42102));
        ...
        let mut network_queue = alice_runtime.initiate_handshakes();
        for _ in 0..8 {
            if network_queue.is_empty() {
                break;
            }
        ...
        let statuses = alice_runtime.peer_statuses();
        assert_eq!(statuses.len(), 1);
        assert_eq!(statuses[0].participant_pubkey, "bob");
        assert!(
            statuses[0].last_handshake_age.is_some(),
            "handshake age should be populated after the exchange"
        );
    }
}
